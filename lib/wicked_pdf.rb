...
class WickedPdf
  ...
  def pdf_from_string(string, options={})
    ...
    command = "\"#{@exe_path}\" #{'-q ' unless on_windows?}#{parse_options(options)} #{parse_page_options(options)} \"file://#{string_file.path}\" \"#{generated_pdf_file.path}\" " # -q for no errors on stdout
